...
import BuAsset from '../../components/assetInquiry/buAsset/buAsset'
...
const mapStateToProps = state=>({
  ...
  buListPageInfo: state.assetInquiry.buListPageInfo,
...
// 日志弹窗
  ...
  render() {
    ...
    const { buListPageInfo } = this.props
    const { showModal, hideModal } = this
    // 表格数据 myAssetsListData 加入序号一栏
    const managerListData = (Object.keys(buListPageInfo).length && buListPageInfo.result.reduce((prev, cur, index) => {
        cur['serialNo'] = index + 1
        return prev.concat(cur)
      }, [])) || []
    const props = { columns, visible, isBuManger, buMangerId, managerListData, buListPageInfo, showModal, hideModal }
    return <BuAsset {...this.props} {...props} />
  }
}
