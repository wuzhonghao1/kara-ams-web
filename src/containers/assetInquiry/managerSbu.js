...
import ManagerSbu from '../../components/assetInquiry/managerSbu/managerSbu'
...
const mapStateToProps = state=>({
  ...
  sbuBordListPageInfo: state.assetInquiry.sbuBordListPageInfo,
...
// 日志弹窗
  ...
  render() {
    ...
    const { sbuBordListPageInfo } = this.props
    const { showModal, hideModal } = this
    // 表格数据 myAssetsListData 加入序号一栏
    const managerListData = (Object.keys(sbuBordListPageInfo).length && sbuBordListPageInfo.result.reduce((prev, cur, index) => {
        cur['serialNo'] = index + 1
        return prev.concat(cur)
      }, [])) || []
    const props = { columns, visible, isBuManger, buMangerId, managerListData, sbuBordListPageInfo, showModal, hideModal }
    return <ManagerSbu {...this.props} {...props} />
  }
}
