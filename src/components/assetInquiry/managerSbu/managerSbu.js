...
class ManagerSbu extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  doExport = () => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      this.props.subordinateExcel(newValues).then(result => {
        this.setState({ exportStaus: false })
        if (result && result.response.resultMessage) {
          message.error(result.response.resultMessage)
        } else {
          saveAs(result.response, `直接下属资产明细表_${this.props.user.accountName}_${Date.parse(new Date())}.xls`)
        }
      })
    })
  }
  ...
  render() {
    const { columns, user, managerListData=[], sbuBordListPageInfo, isBuManger } = this.props
    const { getFieldDecorator } = this.props.form;
    // console.log('用户角色', user.roleInfos[0].infos, user.roleInfos[0].userRole, managerListPageInfo)
    let total; // 总共条数
    if (Object.keys(sbuBordListPageInfo).length) {
      total = sbuBordListPageInfo.count
    }
...
export default Form.create()(ManagerSbu)
